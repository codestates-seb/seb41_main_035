...
public class RedisRepository {
    ...
    public boolean hasRefreshToken(String refreshToken, String tokenSubject) {
        // 해당 회원에 대한 키값이 존재하는 경우에만
        if(Boolean.TRUE.equals(redisTemplate.hasKey(tokenSubject))) {
            ValueOperations<String, Object> map = redisTemplate.opsForValue();
            // 전달받은 RTK 토큰과 저장된 토큰값이 같은지 비교
            String savedRefreshToken = map.get(tokenSubject).toString();
